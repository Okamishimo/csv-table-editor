...
- 儲存格內編輯；可新增或刪除列與欄。
- 可用編輯格線開啟的檔案，可透過工具列的 **Editable / Read-only** 開關切換可編輯與唯讀。預設為可編輯，切換只作用於目前的編輯器工作階段，並保留未儲存的修改。唯讀時仍可搜尋、選取、複製、調整檢視排序與展開多行儲存格，但不能修改儲存格、增刪列欄、復原／取消復原、回復歷史版本或變更編碼；鎖定前的修改仍可儲存。大檔串流預覽維持唯讀。
